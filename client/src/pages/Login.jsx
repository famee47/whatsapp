...
export default function Login() {
  ...
  const submit = async (e) => {
    e.preventDefault();
    if (!form.email.trim() || !form.password) return toast.error('Email/username and password are required.');
    setLoading(true);
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#111b21] px-4" style={{minHeight:'100dvh'}}>
      <div className="w-full max-w-sm">
        <div className="text-center mb-10">
          ...
          <p className="text-[#8696a0] text-sm mt-2">Chat without a phone number</p>
        </div>
        <form onSubmit={submit} className="space-y-3">
          <input
            ...
            className="w-full px-4 py-4 rounded-xl bg-[#202c33] text-[#e9edef] placeholder-[#8696a0] border-2 border-transparent focus:border-[#00a884] outline-none transition-all text-sm"
            required
          />
          <div className="relative">
            <input
              ...
              className="w-full px-4 py-4 rounded-xl bg-[#202c33] text-[#e9edef] placeholder-[#8696a0] border-2 border-transparent focus:border-[#00a884] outline-none transition-all text-sm pr-12"
              required
            />
          ...
          <button type="submit" disabled={loading}
            ...
            {loading ? <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"/>Logging in...</> : 'Log In'}
          </button>
        </form>
        <p className="text-center text-[#8696a0] text-xs mt-4">
          You can log in with your <span className="text-[#00a884]">email</span> or <span className="text-[#00a884]">username</span>
        </p>
        <p className="text-center text-[#8696a0] text-sm mt-4">
          No account?{' '}
          <Link to="/register" className="text-[#00a884] hover:text-[#00c79a] font-semibold transition-colors">Create one</Link>
        </p>
      </div>
    </div>
  );
