...
const getDisplayName = (u) => u?.displayName?.trim() || u?.username || '';
const maskEmail = (email, isOwn) => {
  if (!email) return '';
  if (isOwn) return email;
...
export default function Home() {
  ...
  const [chatSwipe, setChatSwipe] = useState({}); // chatId -> swipeX (negative = swiped left)
  const chatSwipeStart = useRef({});
  ...
  const handlePullMove = (e) => {
    ...
    pullDist.current = Math.max(0, Math.min(dy, 80));
  };
  const handlePullEnd = async () => {
    if (pullDist.current > 50) {
      setRefreshing(true);
      await loadAll();
      setRefreshing(false);
    }
  ...
  /* ── Chat list swipe-left (mobile) to reveal action buttons ── */
  const handleChatSwipeStart = (e, id) => {
    const t = e.touches?.[0];
    if (!t) return;
    chatSwipeStart.current[id] = { x: t.clientX, y: t.clientY };
  };
  const handleChatSwipeMove = (e, id) => {
    const start = chatSwipeStart.current[id];
    if (!start) return;
    const t = e.touches?.[0];
    const dx = t.clientX - start.x;
    const dy = Math.abs(t.clientY - start.y);
    // If more vertical than horizontal, don't capture (let scroll happen)
    if (dy > Math.abs(dx)) return;
    if (dx < 0) {
      e.preventDefault(); // prevent scroll when swiping left
      setChatSwipe(p => ({ ...p, [id]: Math.max(dx, -140) }));
    } else if (chatSwipe[id] < 0) {
      setChatSwipe(p => ({ ...p, [id]: Math.min(dx + (p[id]||0), 0) }));
    }
  };
  const handleChatSwipeEnd = (e, id) => {
    const x = chatSwipe[id] || 0;
    if (x < -60) {
      setChatSwipe(p => ({ ...p, [id]: -140 })); // snap open
    } else {
      setChatSwipe(p => ({ ...p, [id]: 0 })); // snap close
    }
    delete chatSwipeStart.current[id];
  };
  const closeChatSwipe = (id) => setChatSwipe(p => ({ ...p, [id]: 0 }));
  ...
  // ── Android back button — go to chat list instead of closing app ──
  useEffect(() => {
    const handleBack = (e) => {
      if (mobileView === 'chat') {
        e.preventDefault();
    ...
    // Push initial state so we can catch first back press
    window.history.pushState({ page: mobileView === 'chat' ? 'chat' : 'list' }, '');
    ...
    const cleanups = [
      handle('newMessage', (msg) => {
        ...
        setConversations(prev => {
          ...
          if (exists) {
            // Update existing conversation — move to top only on new message
            return prev.map(c=>c._id===cid?{...c,lastMessage:msg,updatedAt:msg.createdAt}:c)
              .sort((a,b)=>new Date(b.updatedAt)-new Date(a.updatedAt));
          } else {
            // Conversation was deleted — fetch fresh conv and add back (no old messages)
            convAPI.getAll().then(r => {
              const all = Array.isArray(r.data) ? r.data : [];
              setConversations(all);
            }).catch(()=>{});
            // Clear messages for that conv so old ones don't appear if user opens it
            setMessages(prev2 => activeChatRef.current?.data?._id === cid ? [] : prev2);
            return prev;
          }
        });
  ...
  const startDM = async (otherUser) => {
    try {
      ...
      setConversations(prev => {
        ...
        // Insert in sorted order by updatedAt, not always at top
        return [...prev, data].sort((a,b)=>new Date(b.updatedAt)-new Date(a.updatedAt));
      });
      openDM(data);
  ...
  const handleTouchMoveSwipe = (e, msg, isMine) => {
    ...
    if (isMine) {
      ...
      // Others' messages: swipe RIGHT (positive dx)
      if (dx > 0 && dx < 80) setSwipeState(p=>({...p,[msg._id]:dx}));
    }
  };
  const handleTouchEndSwipe = (e, msg) => {
    const tx = swipeState[msg._id]||0;
    if (Math.abs(tx) > 50) { setReplyingTo(msg); setTimeout(()=>textareaRef.current?.focus(),100); }
  ...
  const deleteChatForMe = async (chatId) => {
    try {
      // Delete chat record + permanently clear all messages for me
      await userAPI.deleteChat(chatId);
      await convAPI.clearHistory(chatId).catch(()=>{}); // clear messages server-side
  ...
  const getContactName = (uid, fallbackUsername, userObj=null) => {
    const c = contacts.find(c=>(c.user?._id||c.user)===uid);
    if (c?.nickname) return c.nickname;
    if (userObj?.displayName?.trim()) return userObj.displayName.trim();
  ...
  const activeChatName = activeChat?.type==='dm'
    ? getContactName(activeChatUser?._id, activeChatUser?.username, activeChatUser)
        ...
        <div className="flex items-center justify-between px-3 py-3 bg-[#202c33] flex-shrink-0">
          <button onClick={()=>{setShowProfile(true);setProfileUser(null);}} className="flex items-center gap-2 hover:opacity-80 transition-opacity">
            ...
            <div className="min-w-0">
              <p className="text-white font-semibold text-sm truncate max-w-[120px]">{getDisplayName(user)}</p>
              <p className="text-[#8696a0] text-xs truncate max-w-[120px]">@{user?.username}</p>
            </div>
          </button>
          ...
          <div className="flex-1 overflow-y-auto scrollbar-hide"
            ...
            {refreshing && (
              <div className="flex items-center justify-center py-3 gap-2">
                <span className="w-4 h-4 border-2 border-[#00a884]/30 border-t-[#00a884] rounded-full animate-spin"/>
                <span className="text-[#8696a0] text-xs">Refreshing...</span>
              </div>
            )}
            ...
            {!loading && allChatItems.map(item => {
              ...
              const displayName = isDM ? getContactName(other?._id, other?.username, other) : item.name;
              const pinned = isPinned(item._id);
              const muted = isMuted(item._id);
              const hasDraft = drafts[item._id] && !isActive;
              return (
                <div key={item._id} className="relative overflow-hidden border-b border-[#2a3942]/20">
                  {/* Swipe-left action buttons (mobile only) */}
                  <div className="absolute right-0 top-0 bottom-0 flex items-stretch z-10"
                    style={{width:'140px', transform: `translateX(${140 + (chatSwipe[item._id]||0)}px)`, transition: chatSwipeStart.current[item._id] ? 'none' : 'transform 0.2s ease'}}>
                    <button onTouchStart={e=>e.stopPropagation()} onClick={e=>{e.stopPropagation();closeChatSwipe(item._id);
                      const cid=item._id;
                      if(lockedChats.includes(cid)){userAPI.lockChat(cid,null).then(r=>{setLockedChats(r.data.lockedChats||[]);toast.success('Unlocked');}).catch(()=>toast.error('Failed'));}
                      else{setPinLockModal(cid);setPinInput('');setPinError('');}
                    }} className="flex flex-col items-center justify-center w-[46px] bg-[#5c6bc0] text-white text-xs gap-1">
                      <span className="text-base">{lockedChats.includes(item._id)?'🔓':'🔒'}</span>
                      <span>{lockedChats.includes(item._id)?'Unlock':'Lock'}</span>
                    </button>
                    <button onTouchStart={e=>e.stopPropagation()} onClick={e=>{e.stopPropagation();closeChatSwipe(item._id);
                      const cid=item._id; const isMuted2=isMuted(cid);
                      if(isMuted2){userAPI.muteChat(cid,'unmute').then(()=>toast.success('Unmuted')).catch(()=>toast.error('Failed'));}
                      else{userAPI.muteChat(cid,'8h').then(()=>toast.success('Muted 8h')).catch(()=>toast.error('Failed'));}
                    }} className="flex flex-col items-center justify-center w-[46px] bg-[#607d8b] text-white text-xs gap-1">
                      <span className="text-base">{isMuted(item._id)?'🔔':'🔇'}</span>
                      <span>{isMuted(item._id)?'Unmute':'Mute'}</span>
                    </button>
                    <button onTouchStart={e=>e.stopPropagation()} onClick={e=>{e.stopPropagation();closeChatSwipe(item._id);deleteChatForMe(item._id);}} className="flex flex-col items-center justify-center w-[48px] bg-red-500 text-white text-xs gap-1">
                  ...
                  {/* Main chat row */}
                  <button
                  onClick={()=>{closeChatSwipe(item._id);isDM?openDM(item):openGroup(item);}}
                  onContextMenu={e=>{e.preventDefault();e.stopPropagation();setChatCtxMenu({x:e.clientX,y:e.clientY,item,isDM});}}
                  onTouchStart={e=>{handleChatSwipeStart(e,item._id);}}
                  onTouchMove={e=>{handleChatSwipeMove(e,item._id);}}
                  onTouchEnd={e=>{handleChatSwipeEnd(e,item._id);}}
                  style={{transform:`translateX(${chatSwipe[item._id]||0}px)`, transition: chatSwipeStart.current[item._id] ? 'none' : 'transform 0.2s ease'}}
                  className={`w-full flex items-center gap-3 px-3 py-3 hover:bg-[#2a3942] transition-colors ${isActive?'bg-[#2a3942]':''} text-left`}>
            ...
            <div className="flex items-center gap-3 px-4 py-3 bg-[#202c33] border-b border-[#2a3942] flex-shrink-0">
              ...
              <button onClick={()=>{setProfileUser(activeChatUser||activeChat.data);setShowProfile(true);}} className="flex items-center gap-3 flex-1 min-w-0 hover:opacity-80 transition-opacity text-left">
                ...
                <div className="min-w-0">
                  ...
                  {activeChat.type==='dm' && activeChatUser?.displayName?.trim() && (
                    <p className="text-[#8696a0] text-xs leading-none truncate">@{activeChatUser?.username}</p>
                  )}
                ...
                return (
                  <div key={msg._id||i}>
                    ...
                    <div
                      ...
                      onTouchMove={e=>!msg.isDeleted&&handleTouchMoveSwipe(e,msg,isMine)}
                ...
                <>
                  ...
                  <div className="w-full space-y-2">
                    ...
                    <div className="flex items-center gap-3 px-3 py-2 bg-[#202c33] rounded-xl">
                      ...
                      <div>
                        <p className="text-[#8696a0] text-xs">Joined</p>
                        <p className="text-[#e9edef] text-sm">{(profileUser||user)?.createdAt ? new Date((profileUser||user).createdAt).toLocaleDateString('en-US',{day:'numeric',month:'long',year:'numeric'}) : 'Unknown'}</p>
                      </div>
                    </div>
                    {/* Email — full for own, masked for others */}
                    {(profileUser||user)?.email && (
                      <div className="flex items-center gap-3 px-3 py-2 bg-[#202c33] rounded-xl">
                        <span className="text-[#8696a0] text-lg">📧</span>
                        <div>
                          <p className="text-[#8696a0] text-xs">Email</p>
                          <p className="text-[#e9edef] text-sm">{maskEmail((profileUser||user).email, !profileUser)}</p>
                        </div>
                      </div>
                    )}
                  </div>
